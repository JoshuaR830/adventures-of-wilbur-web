...
export async function getServerSideProps() {
  ...
  const res = await fetch(`https://localhost:5001/GetMostRecentIndex`, {'mode': 'cors'});
  
  const res2 = await fetch(`https://localhost:5001/WilburPagedItemNewestFirst?page=1&limit=1`, {'mode': 'cors'});
  console.log(res)
...
const getKey = (pageIndex, previousPageData) => {
  ...
  if (pageIndex === 0 && previousPageData === null) {
    return `https://localhost:5001/WilburPagedItemNewestFirst?page=${pageIndex}&limit=${6}`
  }
